...
py = cv2.threshold(ref, 0, 255, cv2.THRESH_BINARY_INV |
	cv2.THRESH_OTSU)[1]
...
for d in groupCnts:
	...
	if(h < (ano_h)/len(groupCnts)):
		...
		#group = clone[y - 2:y + h + 2, x - 2:x + w + 2]
	 	group = ref[y - 6 :y + h , x - 6 :x + w]
		#cv2.imshow("shit", group)
		#cv2.waitKey(0)
		height, width = group.shape[:2]
		group = cv2.resize(group, (width*5, height*5), interpolation = cv2.INTER_CUBIC)
	...
	for b in anoCnts:
		...
		if(h < (sum_h)/len(refCnts)):
			...
			#cv2.rectangle(clone, (x, y), (x + w, y + h), (0, 255, 0), 2)
			#group = clone[y - 2:y + h + 2, x - 2:x + w + 2]
	 		group = clone[y-1:y + h+2 , x-1 :x + w+2]
			group = cv2.bitwise_not(group)
			group = cv2.GaussianBlur(group,(9,9),100)
